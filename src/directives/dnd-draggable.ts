...
export class DndDraggable implements OnInit, OnDestroy {
    private optionP: DndDraggableConfig;


    public get option(): DndDraggableConfig {
        return this.optionP;
    }


    @Input('dndDraggable')
    public set option(v: DndDraggableConfig | undefined) {
        if (v)
            this.optionP = v;
        else
            this.optionP = <DndDraggableConfig>{ draggable: true }
    }
    ...
    public handleDragStart(event: DragEvent): void {
        ...
        try {
            ...
            try {
                ...
                event.dataTransfer.effectAllowed = effectsAllowed[0] as DataTransfer["effectAllowed"];
