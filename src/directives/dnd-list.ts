...
export class DndList implements OnInit, OnDestroy {
    private optionP: DndListSettings;

    @Input('dndList')
    public set option(v : DndListSettings|undefined) {
        if(v)
        this.optionP = v;
        else
        this.optionP={
    ...
    public get option() : DndListSettings {
        return this.optionP;
    }
    ...
    public handleDragOver(event: DragEvent): boolean {
        ...
        if (!ignoreDataTransfer) {
            event.dataTransfer.dropEffect = dropEffect as DataTransfer["dropEffect"];
        }
